...
if(_idxs){
  _data.value = idTypeArr.includes(testId) ? list[_idxs.idx] : pictestList[_idxs.idx];
...
const handleBtn = (_num:any,_Itemdata:any) =>{
  ...
  if(acQuestions.value < _data.value.questions.length - 1){
    ...
    if(idTypeArr.includes(testId)){
      ...
      location.href = '/vision-health-test/result?id=' + testId + '&sum=' + sumNum.value + '&leftSum=' + leftSum.value + '&rightSum=' + rightSum.value
    }
  }
}
const handleNextBtn = () =>{
...
<template>
  <div class="pictestPage">
    ...
    <div class="pictestPage-content">
      ...
      <div class="pictestPage-content-closeeye" v-else-if="_data.questions[acQuestions].type === 'closeeye'">
      ...
      <div class="pictestPage-content-picQuestion" v-else-if="_data.questions[acQuestions].type === 'picQuestion'">
        <div>
          {{_data.questions[acQuestions].question}}
        </div>
        <div>
          <div v-for="(item,index) in _data.questions[acQuestions].options" :key="index" @click.stop="handleBtn(_data.questions[acQuestions].imageOptions[index],_data.questions[acQuestions])">
            <img :src="item" alt="">
          </div>
        </div>
      </div>
      <div class="pictestPage-content-question" v-else>
        ...
        <div>
          <div v-for="(item,index) in _data.questions[acQuestions].options" :key="index" @click.stop="handleBtn(_data.questions[acQuestions].score[index],_data.questions[acQuestions])">{{item}}</div>
        </div>
      </div>
    </div>
  </div>
